...
public sealed class Plugin : IDalamudPlugin
{
        ...
        try
        {
            if (ConfigurationManager.Configuration.PluginEnabled)
            {
                ...
                if (_gameObjectMovementHook == null)
                {
                    ...
                    _gameObjectMovementHook = Hook<GameObjectMovementDelegate>.FromAddress(movementAddress, OnGameObjectMove);
    ...
    private void ApplyByCommand(string args)
    {
        ...
        try
        {
            ...
            (charaName, profName) = args.Split(',') switch
            {
                var a => (a[0].Trim(), a[1].Trim()),
            };
