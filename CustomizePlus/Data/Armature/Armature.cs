...
public unsafe class Armature
{
                    ...
                    for (var boneIndex = 0; boneIndex < currentPose->Skeleton->Bones.Length; ++boneIndex)
                    {
                        if (currentPose->Skeleton->Bones[boneIndex].Name.String is string boneName && boneName != null)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    PluginTransform = Profile.Bones.TryGetValue(boneName, out var bt) && bt.IsEdited()
                                        ? bt
                                        : new BoneTransform(),
                                };
                            }
        ...
        for (var pSkeleIndex = 1; pSkeleIndex < Skeleton->PartialSkeletonCount; ++pSkeleIndex)
        {
            ...
            for (var poseIndex = 0; poseIndex < 4; ++poseIndex)
            {
                ...
                if (currentPose != null && partialSkele.ConnectedBoneIndex >= 0)
                {
                    ...
                    {
                        currentPose->ModelPose[i] =
                            ApplyPropagatedTransform(currentPose->ModelPose[i], transB, transA->Translation, transB->Rotation);
                        currentPose->ModelPose[i] =
                            ApplyPropagatedTransform(currentPose->ModelPose[i], transB, transB->Translation, transA->Rotation);
                    }
                }
            }
        }
    ...
    private hkQsTransformf ApplyPropagatedTransform(hkQsTransformf init, hkQsTransformf* propTrans, hkVector4f initialPos,
        hkQuaternionf initialRot)
    {
        ...
        {
            Translation = Vector3.Transform(init.Translation.GetAsNumericsVector().RemoveWTerm() - sourcePosition, deltaRot)
                .ToHavokTranslation(),
