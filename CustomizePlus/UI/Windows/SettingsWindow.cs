...
internal class SettingsWindow : WindowBase
{
    ...
    private void DrawPluginEnabledCheckbox()
    {
        ...
        if (CtrlHelper.CheckboxWithTextAndHelp("##pluginenabled", "Enable profiles", "Globally enables or disables all profiles.",
                ref isChecked))
        {
    ...
    private void DrawAdvancedSettings()
    {
        ...
        CtrlHelper.LabelWithIcon(FontAwesomeIcon.ExclamationTriangle,
            "Those are advanced settings, no support is provided for those settings unless they are not working at all.");
        ImGui.NewLine();
    ...
    private void DrawEnableRootPositionCheckbox()
    {
        ...
        if (CtrlHelper.CheckboxWithTextAndHelp("##rootpos", "Root position editing", "Enables ability to edit root bone position.",
                ref isChecked))
        {
