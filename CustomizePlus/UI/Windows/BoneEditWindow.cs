...
public class BoneEditWindow : WindowBase
{
    ...
    protected override string Title
        => $"Edit Profile: {profileInProgress.ProfileName}";

    protected override bool SingleInstance
        => true;
    ...
    protected override ImGuiWindowFlags WindowFlags
        => ImGuiWindowFlags.NoScrollbar | (dirty ? ImGuiWindowFlags.UnsavedDocument : ImGuiWindowFlags.None);

    protected override bool LockCloseButton
        => true;

    private Armature? SkeletonInProgress
        => profileInProgress.Armature;
    ...
    protected override void DrawContents()
    {
                ...
                new Vector2(0, ImGui.GetFrameHeightWithSpacing() - 56)))
        {
            ...
            foreach (var boneGroup in groupedBones.OrderBy(x => (int)x.Key))
            {
                ...
                if (boneGroup.Key == BoneData.BoneFamily.Root && !Plugin.ConfigurationManager.Configuration.RootPositionEditingEnabled)
                    continue;
    ...
    {
        var msg =
            $"The editor can't find {profileInProgress.CharacterName} or their bone data in the game's memory.\nCertain editing features will be unavailable.";
